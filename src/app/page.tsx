...
const services: Service[] = [
  {
    ...
    description:
      "방위사업법과 관련 규정을 빠르게 확인할 수 있는 업무형 조회 서비스입니다.",
  ...
  },
  {
    ...
    description:
      "열린국회정보 API를 통해서 국회의원 명단, 회의록 등 국회 관련 정보를 빠르게 조회 가능합니다.",
  ...
  },
  {
    ...
    description:
      "DAPA 직원간의 신뢰를 바탕으로 중고거래 가능한 카카오톡 채팅방입니다.",
  ...
  },
  {
    ...
    tags: ["#슬기로운청사생활", "#복지시설", "#어린이집", "#헬스장", "#약국", "#병원", "#은행", "#이발소"],
...
function HeroQuickButton({ service }: { service: Service }) {
  ...
  if (inactive) {
    return (
      <div className="group flex items-center justify-between gap-4 rounded-[1.5rem] border border-black/10 bg-white/80 px-4 py-4 text-neutral-400">
        <div className="flex items-center gap-4">
          <div
            className={`grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br ${service.accent} text-white`}
          >
        ...
        <span className="rounded-full border border-black/10 bg-black/5 px-4 py-2 text-sm font-medium">
          {service.cta}
        </span>
      </div>
    );
  ...
  return (
    <div className="group flex items-center justify-between gap-4 rounded-[1.5rem] border border-black/10 bg-white/82 px-4 py-4 transition-all duration-200 hover:-translate-y-1 hover:border-black/20 hover:bg-white hover:shadow-[0_16px_36px_rgba(0,0,0,0.08)]">
      <div className="flex items-center gap-4">
        <div
          className={`grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br ${service.accent} text-white`}
        >
      ...
      <Link
        ...
        className="inline-flex shrink-0 items-center gap-2 rounded-full border border-black bg-black px-4 py-2 text-sm font-medium !text-white transition-colors duration-200 hover:bg-neutral-800"
      >
        {service.cta}
...
function ServiceCard({ service }: { service: Service }) {
  ...
  return (
    <article className="rounded-[2rem] border border-black/10 bg-[#fffaf2]/88 p-6 shadow-[0_18px_40px_rgba(0,0,0,0.06)] transition-all duration-200 hover:-translate-y-1 hover:border-black/20 hover:bg-white hover:shadow-[0_22px_48px_rgba(0,0,0,0.08)]">
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
        ...
        {service.href ? (
          <Link
            ...
            target="_blank"
            rel="noreferrer noopener"
            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-black bg-black px-5 py-3 text-sm font-medium !text-white transition-colors duration-200 hover:bg-neutral-800"
          >
            {service.cta}
            <ExternalLink className="h-4 w-4" />
          </Link>
        ) : (
          <span className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-black/10 bg-black/5 px-5 py-3 text-sm font-medium text-neutral-500">
            {service.cta}
          </span>
        )}
      </div>
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(205,180,219,0.14),transparent_26%),linear-gradient(180deg,#f8f2e8_0%,#f4efe6_46%,#efe6d8_100%)] text-neutral-950">
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="relative overflow-hidden rounded-[2.5rem] border border-black/10 bg-[#fffaf2]/85 px-6 py-10 shadow-[0_30px_80px_rgba(50,42,33,0.08)] backdrop-blur md:px-10 md:py-14 lg:px-14 lg:py-16">
        ...
        <section
          id="services"
          className="mt-14 rounded-[2.3rem] border border-black/10 bg-[#fffaf2]/80 px-6 py-8 md:px-8 md:py-10"
        >
        ...
        <section
          id="shortcut"
          className="mt-14 rounded-[2.2rem] border border-black/10 bg-[#f7f0e6] px-6 py-8 md:px-8"
        >
          <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              ...
              <p className="mt-4 max-w-xl text-sm leading-7 text-neutral-700">
                자주 쓰는 경우 홈 화면에 추가해 두면 더 빠르게 접근할 수
                있습니다.
              </p>
            </div>
